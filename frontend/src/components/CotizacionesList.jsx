...
const FacturaDetailsModal = ({ cotizacion, onClose, token }) => {
    ...
    const downloadFile = async (docType) => {
        ...
        try {
            ...
            // --- CORRECCIÓN ---
            // Ahora solo enviamos el ID de la cotización.
            // El backend se encargará de reconstruir todo.
            const payload = { cotizacion_id: cotizacion.id };
            ...
            a.download = `Comprobante_${cotizacion.numero_cotizacion}.${extension}`;
    ...
    const sunatResponse = cotizacion.factura?.sunat_response;
        ...
        >
            ...
            >
                <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-4 border-b pb-2">
                    Detalles del Comprobante N° {cotizacion.numero_cotizacion}
                </h2>
                {sunatResponse ? (
                                ...
                                {filteredCotizaciones.map((cot) => (
                                    <tr key={cot.id} className="hover:bg-gray-100 dark:hover:bg-gray-700/50 even:bg-gray-50 dark:even:bg-gray-800/50 transition-colors duration-200">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-center">
                                            {cot.factura ? (
                                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${cot.factura.success ? 'bg-green-100 text-green-800 dark:bg-green-800/50 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-800/50 dark:text-red-300'}`}>
                                                    {cot.factura.success ? 'Facturado' : 'Rechazado'}
                                                </span>
                                            ) : (
